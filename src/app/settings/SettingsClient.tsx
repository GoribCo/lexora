...
export default function SettingsClient() {
  ...
  function handleExport() {
    ...
    const data = {
      ...
      version: '1',
      progress: (() => { try { return JSON.parse(localStorage.getItem('lexora_progress') || '{}') } catch { return {} } })(),
      srs: (() => { try { return JSON.parse(localStorage.getItem('lexora_srs') || '{}') } catch { return {} } })(),
      streak: (() => { try { return JSON.parse(localStorage.getItem('lexora_streak') || '{}') } catch { return {} } })(),
    ...
    a.href = url
    a.download = `lexora-backup-${today}.json`
    a.click()
  ...
  function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.progress) localStorage.setItem('lexora_progress', JSON.stringify(data.progress))
        if (data.srs) localStorage.setItem('lexora_srs', JSON.stringify(data.srs))
        if (data.streak) localStorage.setItem('lexora_streak', JSON.stringify(data.streak))
        if (data.ratings) localStorage.setItem('lexora_stage_ratings', JSON.stringify(data.ratings))
        if (data.notes) localStorage.setItem('lexora_stage_notes', JSON.stringify(data.notes))
